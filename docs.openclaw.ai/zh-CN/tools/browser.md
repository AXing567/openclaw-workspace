浏览器（OpenClaw 托管） - OpenClaw 跳转到主要内容 OpenClaw home page 简体中文 搜索... ⌘ K 
- GitHub 

- Releases 

- Discord 
搜索... Navigation 浏览器 浏览器（OpenClaw 托管） 快速开始 安装 消息渠道 代理 工具 模型 平台 网关与运维 参考 帮助 
##### 概览 
...
它与你的个人浏览器隔离，通过 Gateway 网关内部的小型本地控制服务进行管理（仅限 loopback）。 新手视角： 
...
## ​ 快速开始 
复制 `openclaw browser --browser-profile openclaw status openclaw browser --browser-profile openclaw start openclaw browser --browser-profile openclaw open https://example.com openclaw browser --browser-profile openclaw snapshot `如果出现”Browser disabled”，请在配置中启用它（见下文）并重启 Gateway 网关。 
## ​ 配置文件： `openclaw `与 `chrome `
...
## ​ 使用 Brave（或其他基于 Chromium 的浏览器） 
如果你的 系统默认 浏览器是基于 Chromium 的（Chrome/Brave/Edge 等），OpenClaw 会自动使用它。设置 `browser.executablePath `可覆盖自动检测： CLI 示例： 复制 `openclaw config set browser.executablePath "/usr/bin/google-chrome" `复制 `// macOS { browser : { executablePath : "/Applications/Brave Browser.app/Contents/MacOS/Brave Browser" } } // Windows { browser : { executablePath : "C:\\Program Files\\BraveSoftware\\Brave-Browser\\Application\\brave.exe" } } // Linux { browser : { executablePath : "/usr/bin/brave-browser" } } `
## ​ 本地控制与远程控制 
...
## ​ Browserless（托管远程 CDP） 
Browserless 是一个托管的 Chromium 服务，通过 HTTPS 暴露 CDP 端点。你可以将 OpenClaw 浏览器配置文件指向 Browserless 区域端点，并使用你的 API 密钥进行认证。 示例： 复制 `{ browser : { enabled : true , defaultProfile : "browserless" , remoteCdpTimeoutMs : 2000 , remoteCdpHandshakeTimeoutMs : 4000 , profiles : { browserless : { cdpUrl : "https://production-sfo.browserless.io?token=<BROWSERLESS_API_KEY>" , color : "#00AA00" , } , } , } , } `注意事项： 
...
## ​ Chrome 扩展中继（使用你现有的 Chrome） 
OpenClaw 还可以通过本地 CDP 中继 + Chrome 扩展驱动 你现有的 Chrome 标签页 （无需单独的”openclaw”Chrome 实例）。 完整指南： Chrome 扩展 流程： 
...
- 加载扩展（开发/未打包）： 
复制 `openclaw browser extension install `
...
可选：如果你想要不同的名称或中继端口，创建你自己的配置文件： 复制 `openclaw browser create-profile \ --name my-chrome \ --driver extension \ --cdp-url http://127.0.0.1:18792 \ --color "#00AA00" `注意事项： 
...
#### ​ Docker Playwright 安装 
如果你的 Gateway 网关在 Docker 中运行，避免使用 `npx playwright `（npm 覆盖冲突）。改用捆绑的 CLI： 复制 `docker compose run --rm openclaw-cli \ node /app/node_modules/playwright-core/cli.js install chromium `要持久化浏览器下载，设置 `PLAYWRIGHT_BROWSERS_PATH `（例如 `/home/node/.cache/ms-playwright `）并确保 `/home/node `通过 `OPENCLAW_HOME_VOLUME `或绑定挂载持久化。参见 Docker 。 
## ​ 工作原理（内部） 
高层流程： 
...
所有命令也接受 `--json `以获得机器可读的输出（稳定的负载）。 基础操作： 
...
这些可以组合使用： 复制 `openclaw browser wait "#main" \ --url "**/dash" \ --load networkidle \ --fn "window.ready===true" \ --timeout-ms 15000 `
## ​ 调试工作流 
...
## ​ JSON 输出 
`--json `用于脚本和结构化工具。 示例： 复制 `openclaw browser status --json openclaw browser snapshot --interactive --json openclaw browser requests --filter api --json openclaw browser cookies --json `JSON 格式的角色快照包含 `refs `加上一个小的 `stats `块（lines/chars/refs/interactive），以便工具可以推断负载大小和密度。 
## ​ 状态和环境开关 
...
## ​ 故障排除 
有关 Linux 特定问题（特别是 snap Chromium），请参阅 浏览器故障排除 。 
## ​ 智能体工具 + 控制工作原理 
智能体获得 一个工具 用于浏览器自动化： 
...
这使智能体保持确定性并避免脆弱的选择器。 Web 工具 浏览器登录 ⌘ I 技术支持 This documentation is built and hosted on Mintlify, a developer documentation platform
